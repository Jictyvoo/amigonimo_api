...
func TestDenylistUseCase(t *testing.T) {
	...
	t.Run("get denylist", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		repo := NewMockRepository(ctrl)
		provider := NewFacadeProvider(NewMockParticipantFacade(ctrl), NewMockSecretFriendFacade(ctrl))
		repo.EXPECT().GetDenyListByParticipant(ParticipantRef{UserID: userID, SecretFriendID: sfID}).Return([]entities.DeniedUser{{ID: mustHexID(t)}}, nil)
	...
	t.Run("add entry validations and success", func(t *testing.T) {
		tests := []struct {
			name    string
			setup   func(*MockRepository, *MockParticipantFacade, *MockSecretFriendFacade)
			...
			},
			{
				...
				target: targetID,
				setup: func(repo *MockRepository, participant *MockParticipantFacade, sf *MockSecretFriendFacade) {
					participant.EXPECT().CheckParticipantInSecretFriend(sfID, userID).Return(entities.Participant{ID: participantID}, nil)
					participant.EXPECT().CheckParticipantInSecretFriend(sfID, targetID).Return(entities.Participant{ID: mustHexID(t)}, nil)
					repo.EXPECT().GetDenyListByParticipant(ref).Return([]entities.DeniedUser{{ID: mustHexID(t)}, {ID: mustHexID(t)}}, nil)
					sf.EXPECT().GetSecretFriendByID(sfID).Return(entities.SecretFriend{ID: sfID, MaxDenyListSize: 2}, nil)
				},
			...
			},
			{
				...
				setup: func(repo *MockRepository, participant *MockParticipantFacade, sf *MockSecretFriendFacade) {
					...
					sf.EXPECT().GetSecretFriendByID(sfID).Return(entities.SecretFriend{ID: sfID, MaxDenyListSize: 2}, nil)
					repo.EXPECT().AddDenyListEntry(ref, targetID).Return(entities.DeniedUser{ID: mustHexID(t)}, nil)
				},
			},
		}
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				ctrl := gomock.NewController(t)
				repo := NewMockRepository(ctrl)
				participant := NewMockParticipantFacade(ctrl)
				sf := NewMockSecretFriendFacade(ctrl)
				if tt.setup != nil {
					tt.setup(repo, participant, sf)
				}
				uc := New(associatedUser, repo, NewFacadeProvider(participant, sf))
