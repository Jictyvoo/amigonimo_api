...
func WithRequest[I any](method, path string, body I) Option {
	return func(r *HttpRunner) {
		...
		r.makeRequest = func(baseURL string, _ runners.RunnerContext) (*http.Request, error) {
...
func WithSubsequentRequest[T, I any](method, path string, bodyGen func(T) I) Option {
	return func(r *HttpRunner) {
		r.makeRequest = func(baseURL string, rCtx runners.RunnerContext) (*http.Request, error) {
			if bodyGen == nil {
				rCtx.Fatal("body generation is disabled")
			}

			previousObject, ok := runners.LoadFromCtx[T](rCtx)
			if !ok {
